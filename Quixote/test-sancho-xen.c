...
static void receive_events(CO(XENduct, duct), CO(Buffer, bufr))
...
static void receive_trajectory(CO(XENduct, duct), CO(Buffer, bufr))
...
static void receive_forensics(CO(XENduct, duct), CO(Buffer, bufr))
...
static _Bool process_command(CO(XENduct, duct), CO(Buffer, bufr))
...

{
	...
	if ( domid == NULL ) {
		fputs("No domain name specified.\n", stderr);
		return 1;
	}
