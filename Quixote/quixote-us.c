 ...
 * Private function.
 *
 * This function carries out the addition of an event description to
 * the forensics log for events that are generated by a process running
 * in an untrusted state.
 *
 * \param log_event	A pointer to the character buffer containing
 *			the ASCII encoded event description.
 *
 * \return	A boolean value is returned to indicate whether
...
static _Bool add_log_event(CO(String, log_event))

{
	...
	/* Parse the event. */
	INIT(NAAAIM, SecurityEvent, event, ERR(goto done));
	if ( !event->parse(event, log_event) )
		ERR(goto done);
	...
	if ( Debug )
		fprintf(Debug, "Log update: status=%d.\n", status);
