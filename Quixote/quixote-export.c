 ...
 * events.  Queued events are specified as a string with linefeeds
...

{
	...
	/* Output is to a file we can simply write the string. */
	if ( Output_File != NULL ) {
	...
	/* For MQTT based output break the message into descriptions. */
	ep = Event->get_event(Event);
	while ( (p = strchr(ep, '\n')) ) {
		*p = '\0';
		if ( !Output_String->add(Output_String, ep) )
			ERR(goto done);
 ...
 * Private function.
 *
 * This function is responsible for outputting the security state event
 * descriptions generated by the modeling namespace.
 *
 * \return		A boolean value is returned to indicate whether
 *			or not processing of the event was successful.  A
 *			false value indicates a failure in event
