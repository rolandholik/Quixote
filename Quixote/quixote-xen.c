 ...
 *
 * \param duct	A pointer to the object used to communcit with
 *		the Xen based TMA.
 *
...
static _Bool add_aggregate(CO(XENduct, duct), String str)
...
static _Bool process_event(CO(XENduct, duct))
...
static _Bool send_list(CO(XENduct, duct), CO(LocalDuct, mgmt), \
...
static _Bool send_map(CO(XENduct, duct), CO(LocalDuct, mgmt), \
		      CO(Buffer, bufr))
...
static _Bool process_command(CO(XENduct, duct), CO(LocalDuct, mgmt), \
			     CO(Buffer, cmdbufr))
...

{
	...
	while ( (opt = getopt(argc, argv, "CPSetuM:c:d:h:m:n:o:s:")) != EOF )
		switch ( opt ) {
			case 'C':
				Mode = cartridge_mode;
				break;
	...
	/* Initialize the TSEM event and control object. */
	...
	if ( !Sancho->init_device(Sancho, domid) ) {
		WHACK(Sancho);
		fprintf(stderr, "quixote-xen: Cannot connect to SanchoXen " \
			"instance %s.\n", domid);
		goto done;
	}
