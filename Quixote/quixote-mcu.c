 ...
 * domain.
 *
 * The security domain state change events are transmitted to a
...
static _Bool process_event(CO(TTYduct, duct), const char *event)
...

{
	...
	for (lp= 0; lp < cnt; ++lp ) {
		es->reset(es);
		if ( !Duct->receive_Buffer(Duct, es) )
			ERR(goto done);
...

{
	...
	while ( (opt = getopt(argc, argv, "CPSetcu:d:h:m:n:o:p:s:")) != EOF )
		switch ( opt ) {
			case 'C':
				Mode = cartridge_mode;
				break;
