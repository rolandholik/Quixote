...
#include "id-client-server.h"
#include "SSLduct.h"


/* Verify library/object header file inclusions. */
#if !defined(IDQUERY_LIBID)
#error Library identifier not defined.
#endif

#if !defined(SSLduct_OBJID)
 ...
 *
 * This is an internal diagnostic function which returns an error
 * string describing the reason for an SSL connection error.
 *
 * \param ssl	A pointer to the SSL connection object on which the
 *		error occured.
 *
 * \param err	The error return generated by the SSL I/O operation.
 *			
 * 
 * \return	A pointer to a character string with a description of
 *		the reason for the failure.
 */

static const char * _get_ssl_error(SSL *connection, int const err)

{
	switch ( SSL_get_error(connection, err) ) {
		case SSL_ERROR_NONE:
			return "No error.";
		case SSL_ERROR_ZERO_RETURN:
			return "SSL connection closed.";
		case SSL_ERROR_WANT_READ:
			return "SSL wants additional read.";
		case SSL_ERROR_WANT_WRITE:
			return "SSL wants additional write.";
		case SSL_ERROR_WANT_CONNECT:
			return "SSL wants additional connection.";
		case SSL_ERROR_WANT_ACCEPT:
		        return "SSL wants additional accept.";
		case SSL_ERROR_WANT_X509_LOOKUP:
			return "Additional X509 lookup requested.";
		case SSL_ERROR_SYSCALL:
			return "Error on SSL related syscall.";
		case SSL_ERROR_SSL:
 ...
 *
 * This method is responsible for initializing the STGTGSVR_Duct_State
...
static void _init_state(const SSLduct_State const S) {
	...
	S->libid = IDQUERY_LIBID;
	S->objid = SSLduct_OBJID;
...
static _Bool load_credentials(const SSLduct const this, const char * const key, \
...

{
	...
	if ( (S->connection = SSL_new(S->context)) == NULL ) {
		err = "SSL connection creation failed.";
		goto done;
	}

	if ( SSL_clear(S->connection) != 1 ) {
		err = "SSL connection clearing failed.";
		goto done;
	}
	...
	if ( SSL_set_fd(S->connection, S->fd) != 1 ) {
		err = "Setting of SSL file descriptor failed.";
		goto done;
	}

	if ( (state = SSL_accept(S->connection)) != 1 ) {
		if ( state == 0 )
			fputs("!Error on SSL accept, code = 0.\n", stderr);
		if ( state < 0 )
			fputs("!Error on SSL accept, negative code.\n", \
			      stderr);
		err = _get_ssl_error(S->connection, state);
		goto done;
	}
...

{
	...
	/* Initiate an SSL connection and listen for a client handshake. */
	if ( (S->connection = SSL_new(S->context)) == NULL ) {
		err = "Error creating new SSL connection object.";
		goto done;
	}
	...
	if ( SSL_set_fd(S->connection, S->sockt) != 1 ) {
		err = "Error setting connection file descriptor.";
		goto done;
	}

	if ( (state = SSL_connect(S->connection)) < 0 ) {
		if ( state == 0 )
			fputs("!Error on SSL connect, code = 0", stderr);
		if ( state < 0 )
			fputs("!Error on SSL connect, negative code", stderr);
		err = _get_ssl_error(S->connection, state);
		goto done;
	}
	if ( state == 1 )
	...
	if ( (verify != X509_V_OK) && (verify != 18) )
		err = "Unable to verify SSL connection.";
...

{
	...
	retn.object_size  = sizeof(struct SSL_Duct);
	retn.state_size   = sizeof(struct SSL_Duct_State);
	if ( !root->init(root, IDQUERY_LIBID, SSLduct_OBJID, &retn) )
		return NULL;
	this	    	  = retn.object;
