/** \file
 * This file implements an object for manipulating identity tokens.
 * The token is composed of two major parts.  The first part is a user
 * specific organizational identity with the second portion being an
 * RSA encrypted anonymized user identity with a token specific key.
 */
...

{
	...
	if ( S->ptid->size(S->ptid) != 256 ) {
		fprintf(stdout, "%s: Patient ID size: %u\n", __func__, \
			S->ptid->size(S->ptid));
		ERR(goto done);
	}

	INIT(NAAAIM, Sha256, mac, ERR(goto done));
	mac->add(mac, S->ptid);
	if ( !mac->compute(mac) )
		ERR(goto done);
